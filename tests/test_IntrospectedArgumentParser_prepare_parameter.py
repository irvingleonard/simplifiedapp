...
prepare_parameter = IntrospectedArgumentParser._prepare_parameter
class TestIntrospectedArgumentParserPrepareCallableParameter(TestCase):
	'''
	Tests for the IntrospectedArgumentParser._prepare_parameter class method
	'''
	...
	def test_empty_parameter(self):
		'''
		Test IntrospectedArgumentParser._prepare_parameter without details
		'''

		expected_result = ({}, [], [])
		self.assertEqual(expected_result, prepare_parameter())
	
	def test_parameter_w_none_default(self):
		'''
		Test IntrospectedArgumentParser._prepare_parameter with a None default
		'''
		
		expected_result = ({'default' : SUPPRESS}, [], [])
		self.assertEqual(expected_result, prepare_parameter(default=None))
	
	def test_parameter_w_str_default(self):
		'''
		Test IntrospectedArgumentParser._prepare_parameter with a string default
		'''
		
		expected_result = ({'default' : 'initial'}, [], [])
		self.assertEqual(expected_result, prepare_parameter(default='initial'))
	
	def test_parameter_w_bool_default(self):
		'''
		Test IntrospectedArgumentParser._prepare_parameter with a boolean default
		'''
		
		expected_false_result = ({'action': 'store_true'}, [], [])
		self.assertEqual(expected_false_result, prepare_parameter(default=False))
		
		expected_true_result = ({'action': 'store_false'}, [], [])
		self.assertEqual(expected_true_result, prepare_parameter(default=True))
	
	def test_parameter_w_sequence_default(self):
		'''
		Test IntrospectedArgumentParser._prepare_parameter with a sequence default
		'''
		
		expected_frozenset_result = ({'action': 'extend', 'default': frozenset({'frozen', 'set'}), 'nargs': '*'}, [], [])
		self.assertEqual(expected_frozenset_result, prepare_parameter(default=frozenset(('frozen', 'set'))))
		
		expected_set_result = ({'action': 'extend', 'default': {'e', 't', 's'}, 'nargs': '*'}, [], [])
		self.assertEqual(expected_set_result, prepare_parameter(default={'s', 'e', 't'}))
		
		expected_tuple_result = ({'action': 'extend', 'default': ('tu', 'ple'), 'nargs': '*'}, [], [])
		self.assertEqual(expected_tuple_result, prepare_parameter(default=('tu', 'ple')))
		
		expected_list_result = ({'action': 'extend', 'default': ['li', 'st'], 'nargs': '*'}, [], [])
		self.assertEqual(expected_list_result, prepare_parameter(default=['li', 'st']))
	
	def test_parameter_w_mapping_default(self):
		'''
		Test IntrospectedArgumentParser._prepare_parameter with a mapping default
		'''
		...
		self.assertEqual(expected_result, prepare_parameter(default={'a' : 'map', 'test' : True}))

	def test_parameter_w_numeric_default(self):
		'''
		Test IntrospectedArgumentParser._prepare_parameter with a numeric default
		'''
		
		expected_int_result = ({'default': 14, 'type': int}, [], [])
		self.assertEqual(expected_int_result, prepare_parameter(default=14))
		
		expected_float_result = ({'default': 3.14, 'type': float}, [], [])
		self.assertEqual(expected_float_result, prepare_parameter(default=3.14))
		
		expected_complex_result = ({'default': (6+12j), 'type': complex}, [], [])
		self.assertEqual(expected_complex_result, prepare_parameter(default=(6+12j)))

	# def test_parameter_w_annotations(self):
	# 	'''
	# 	Test IntrospectedArgumentParser._prepare_parameter with annotations
	# 	'''
	#
	...
	def test_parameter_w_docstring(self):
		...
		self.assertEqual(expected_result_matching, prepare_parameter(docstring=docstring_matching))
		...
		self.assertEqual(expected_result_required_bad, prepare_parameter(docstring=docstring_required_bad))
		...
		self.assertEqual(expected_result_optional_bad, prepare_parameter(docstring=docstring_optional_bad, default=True))
