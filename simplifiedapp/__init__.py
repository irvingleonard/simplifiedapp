...
def callable_args(callable_, call_ = '', skip_self = None):
	...
	# LOGGER.warning('Got signature: %s', dict(zip(('args', 'varargs', 'varkw', 'defaults', 'kwonlyargs', 'kwonlydefaults', 'annotations'),(args, varargs, varkw, defaults, kwonlyargs, kwonlydefaults, annotations))))

	if (skip_self is not None) and (args[0] == skip_self):
		args.pop(0)

	if defaults is not None:
		req_args = {arg : {} for arg in args[:-len(defaults)] if arg not in ('self',)}
		opt_args = {arg : {'default' : arg_default} for arg, arg_default in dict(zip(args[-len(defaults):], defaults)).items()}
	else:
		req_args = {arg : {} for arg in args if arg not in ('self',)}
		opt_args = {}
	...
	if varargs is not None:
		arguments.update({varargs : param_metadata(varargs, {'action' : 'extend', 'nargs' : '*'}, annotations, docstring = '')})

	if varkw is not None:
		varkw_arg = '--{}'.format(varkw)
		arguments.update({varkw_arg : param_metadata(varkw, {'action' : 'extend', 'nargs' : '+'}, annotations, docstring = '')})
		if 'help' not in arguments[varkw_arg]:
			arguments[varkw_arg]['help'] = ''
		arguments[varkw_arg]['help'] += '(Use the key=value format for each entry)'
...
def class_args(class_):
	...
	init = getattr(class_, '__init__')
	args, varargs, varkw, defaults, kwonlyargs, kwonlydefaults, annotations = inspect.getfullargspec(init)
	# LOGGER.warning('Got __init__ signature: %s', dict(zip(('args', 'varargs', 'varkw', 'defaults', 'kwonlyargs', 'kwonlydefaults', 'annotations'),(args, varargs, varkw, defaults, kwonlyargs, kwonlydefaults, annotations))))
	if hasattr(init, '__objclass__') and (init.__objclass__ == object):
	...
	if hasattr(class_, '__init__'):
		class_opts.update(callable_args(getattr(class_, '__init__'), call_ = class_, skip_self = 'self'))

	subparsers = {}
	for name, callable_ in inspect.getmembers(class_, inspect.isroutine):
		if (name[0] == '_') and (name not in ('__call__',)):
			continue
		
		try:
			args = callable_args(callable_, call_ = name, skip_self = 'self')
		except ValueError:
			pass
		else:
	...
	if not subparsers:
		raise ValueError('The class MUST have a compatible method')
...
def module_args(module):
	...
	for name, callable_ in inspect.getmembers(module, inspect.isroutine):
		...
		try:
			...
			# print('Uncompatible callable: ' + name)
			pass
		else:
			subparsers[name] = (([], {}), args)
	...
	for name, class_ in inspect.getmembers(module, inspect.isclass):
		try:
			...
			# print('Uncompatible class: ' + name)
			pass
		else:
			subparsers[name] = (([], {}), args)

	if not subparsers:
		raise RuntimeError("The module doesn't have a compatible class or function")
...
# 				LOGGER.warning('Adding subparser %s with: %s', key, subparser_info)
			...
			# LOGGER.warning('Skipping parser metadata')
			pass
		else:
...
def main(target = None, sys_argv = None):
	...
	if target is None:
		target = caller
		# print('Target not defined, using caller as target')
	elif isinstance(target, str):
		# print('Identifying string defined target')
		if target in sys.modules:
			# print('Target is a loaded module')
			target = sys.modules[target]
		elif hasattr(caller, target):
			# print('Target is a member of caller')
			target = getattr(caller, target)
		else:
			try:
				target = __import__(target)
				# print('Target is a loadable module')
